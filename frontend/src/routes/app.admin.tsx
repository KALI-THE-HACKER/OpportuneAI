...
function AdminPage() {
  ...
  return (
    <>
      <PageHeader title="System & ingestion" description="Pipeline status, providers, workers, and the live queue." />

      {isLoading ? <LoadingState /> : isError ? <ErrorState onRetry={() => { stats.refetch(); providers.refetch(); workers.refetch(); queue.refetch(); }} /> : (
        <div className="space-y-8">
          <section className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <StatCard label="Uptime" value={`${stats.data!.uptimePct}%`} progress={stats.data!.uptimePct} />
          </section>

          <section>
            <h2 className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-3">Providers</h2>
            <div className="bg-card ring-1 ring-border rounded-lg overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-surface text-muted-foreground text-xs uppercase tracking-wider">
                  <tr><th className="px-4 py-3 text-left font-medium">Provider</th><th className="px-4 py-3 text-left font-medium">Status</th><th className="px-4 py-3 text-left font-medium">Jobs today</th><th className="px-4 py-3 text-left font-medium">Success rate</th><th className="px-4 py-3 text-left font-medium">Last sync</th></tr>
                </thead>
                <tbody className="divide-y divide-border">
                  {providers.data!.map((p) => (
                    <tr key={p.id}>
                      <td className="px-4 py-3 font-medium">{p.name}</td>
                      <td className="px-4 py-3"><span className={`px-2 py-0.5 text-[10px] font-semibold uppercase rounded ring-1 ${statusTone[p.status]}`}>{p.status}</span></td>
                      <td className="px-4 py-3 text-muted-foreground">{p.jobsToday.toLocaleString()}</td>
          ...
          <section>
            <h2 className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-3">Worker nodes</h2>
              ...
              {workers.data!.map((w) => (
                <div key={w.id} className="p-4 bg-card ring-1 ring-border rounded-lg">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`px-1.5 py-0.5 text-[10px] font-semibold uppercase rounded ring-1 ${statusTone[w.status]}`}>{w.status}</span>
                  </div>
                  <div className="text-xs text-muted-foreground mb-3">{w.region}</div>
                  <Bar label="CPU" value={w.cpu} />
                  <Bar label="Memory" value={w.memory} />
                  <div className="mt-2 text-[11px] text-muted-foreground">{w.jobsProcessed.toLocaleString()} jobs</div>
                </div>
              ))}
            </div>
          </section>
          ...
          <section>
            <h2 className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-3">Live queue</h2>
            <div className="bg-card ring-1 ring-border rounded-lg overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-surface text-muted-foreground text-xs uppercase tracking-wider">
                  <tr><th className="px-4 py-3 text-left font-medium">ID</th><th className="px-4 py-3 text-left font-medium">Provider</th><th className="px-4 py-3 text-left font-medium">Type</th><th className="px-4 py-3 text-left font-medium">Attempts</th><th className="px-4 py-3 text-left font-medium">Status</th><th className="px-4 py-3 text-left font-medium">Enqueued</th></tr>
                </thead>
                <tbody className="divide-y divide-border">
                  {queue.data!.map((q) => (
                    <tr key={q.id}>
                      ...
                      <td className="px-4 py-3"><span className={`px-2 py-0.5 text-[10px] font-semibold uppercase rounded ring-1 ${statusTone[q.status]}`}>{q.status}</span></td>
...
function Bar({ label, value }: { label: string; value: number }) {
  return (
    <div className="mb-1.5">
      <div className="flex items-center justify-between text-[10px] text-muted-foreground mb-0.5"><span>{label}</span><span>{value}%</span></div>
      <div className="h-1 w-full bg-muted rounded-full overflow-hidden">
        <div className={`h-full ${value > 80 ? "bg-destructive" : value > 60 ? "bg-amber-500" : "bg-accent"}`} style={{ width: `${value}%` }} />
      </div>
    </div>
  );
}